...
class get_url:
    ...
    def scrap(self,url):
        try: req   = bs(self.xyz.get(url,headers=self.head).content,'html.parser'); raq   = str(req)[:100000].replace('\\','').replace('u003C','').replace('amp;','').replace('gt;','').replace('lt;','').replace('&&','&').replace('','')
